...
- (void)viewDidLoad {
    ...
    //代码构建文字输入框
    inputView = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-50-64, [UIScreen mainScreen].bounds.size.width, 50)];
    ...
    //添加视图
    [self.tabBarController.tabBar insertSubview:inputView atIndex:0];
    //实例化程序委托类
    app = [UIApplication sharedApplication].delegate;
    self.commentArray = [[NSMutableArray alloc] initWithObjects:nil, nil];
...
- (void)setupHeader
{
    ...
    refreshHeader.beginRefreshingOperation = ^{
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            //构造虚拟数据
            self.subjectDic = [NSDictionary dictionaryWithObjectsAndKeys:@"万建伟",@"username",@"portrait.png",@"userPortrait",@"2014-12-23",@"time",@"在瑞环山",@"title",@"main_ad.png",@"bgView",@"我说的如果阿道adegadfgasdfgaerfg夫两个人的时候两个人噶尔得换个色老头格式化速sdfrgsdfhgsdfsdfghszdfgth度和格式的分公司和收到公司的分公司",@"content",@"12",@"praiseNum", nil];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section ==0) {
        ...
        //时间
        UILabel *time = [[UILabel alloc] initWithFrame:CGRectMake(220, 5, [UIScreen mainScreen].bounds.size.width-32-220, 20)];
        ...
        commentViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"commentTableCell"];
        ...
        cell.floorNum.text = [NSString stringWithFormat:@"%iF",indexPath.row+1];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section ==0) {
        ...
        CGRect rect = [content boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-137, 500) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:15]} context:nil];
...
-(UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (section == 0) {
        UIView * view1 = [[UIView alloc] initWithFrame:CGRectMake(20, 0, [UIScreen mainScreen].bounds.size.width-40, 40)];
        //添加......................
        ...
        //增加点赞数显示
        UIButton * praise = [[UIButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-40-80, 10, 20, 20)];
        ...
        //添加点赞数label
        UILabel * praiseNum = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-40-55, 10, 55, 20)];
        praiseNum.font = [UIFont boldSystemFontOfSize:12];
        praiseNum.textAlignment = NSTextAlignmentLeft;
